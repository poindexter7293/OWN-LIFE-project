...
CREATE TABLE `daily_summary` (
  `daily_summary_id` bigint unsigned NOT NULL AUTO_INCREMENT,
  `member_id` bigint unsigned NOT NULL,
  `summary_date` date NOT NULL,
  `weight_kg` decimal(5,2) DEFAULT NULL,
  `total_eat_kcal` decimal(8,2) NOT NULL DEFAULT '0.00',
  `total_burned_kcal` decimal(8,2) NOT NULL DEFAULT '0.00',
  `total_carb_g` decimal(8,2) NOT NULL DEFAULT '0.00',
  ...
  UNIQUE KEY `uk_daily_summary_member_date` (`member_id`,`summary_date`),
  KEY `idx_daily_summary_member_date` (`member_id`,`summary_date`),
  CONSTRAINT `fk_daily_summary_member` FOREIGN KEY (`member_id`) REFERENCES `member` (`member_id`) ON DELETE CASCADE
...
-- Dump completed on 2026-03-31 14:32:25
